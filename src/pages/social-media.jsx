...
import { index } from "@/store/SocialMediaSlice";
import { useDispatch, useSelector } from "react-redux";
const Page = () => {
  const dispatch = useDispatch();
  const { all, loading, count } = useSelector(({ socialMedia }) => socialMedia);
  ...
  return (
    <>
      <Head>
        <title>{`${process.env.APP_NAME} | Social Media`}</title>
      </Head>
      <Box
        component="main"
        sx={{
                ...
                <CardContent>
                  <Box sx={{ maxWidth: 500 }}>
                    <TextField
                      ...
                      placeholder={t("search_social_media")}
                      variant="outlined"
                    />
                  </Box>
                </CardContent>
